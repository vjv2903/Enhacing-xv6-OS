...
int pop_front(int x) 
{
  ...
  int data;
  data = queue[x][front[x]];
  front[x] = (front[x] + 1) % 10000;
  return data;
}
...
scheduler(void)
{
  ...
  while(1)
  {
    ...
    for(i = 0; i < 4; i++)
    {
      ...
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++)
      {
        ...
        if(queue[i][front[i]] == p->pid) 
        {
          ...
          switchkvm();
          c->proc = 0;
          p->ticks[p->level] = p->ticks[p->level] + 1;
          p->last_run = ticks;
          int fl = 0;
          if(p->curr_ticks >= (1 << i)) 
          {
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; ++p)
    {
      if(p->state == RUNNABLE && ticks - p->last_run - 1000 > 0 && p->level > 0  && 0 < 4 - p->level) 
      {
        for(i = front[p->level]; 0 < rear[p->level] - i; ++i) 
        {
          if(queue[p->level][i] == p->pid) 
          {
            for(j = i+1; 0 < rear[p->level] - j; ++j)
            {
        ...
        rear[p->level] = rear[p->level] - 1;
        p->level = p->level - 1;
        push_back(p->pid, p->level);
      }
      else if(p->state == RUNNABLE && p->level - 4 == 0 && ticks - p->last_run - 1000 > 0) 
      {
        push_back(p->pid, 3);
      }
    }
